...
class camera {
  constructor(vec3_lookfrom, vec3_lookat, vec3_vup, float_vfov, float_aspect) {
    ...
    var vec3_w = (vec3_lookfrom.sub(vec3_lookat)).unit_vector();
    var vec3_u = vec3_vup.cross(vec3_w).unit_vector();
    var vec3_v = vec3_w.cross(vec3_u);
    
    // this.vec3_lower_left_corner = new vec3(-float_half_width, -float_half_height, -1.0);
    this.vec3_lower_left_corner = this.vec3_origin
      .sub(vec3_u.mulfl(float_half_width))
      .sub(vec3_v.mulfl(float_half_height))
      .sub(vec3_w);
      
    //this.vec3_horizontal = new vec3(2*float_half_width, 0.0, 0.0);
    this.vec3_horizontal = vec3_u.mulfl(2*float_half_width);
    
    //this.vec3_vertical = new vec3(0.0, 2*float_half_height, 0.0);
    this.vec3_vertical = vec3_v.mulfl(2*float_half_height);
    
    //this.vec3_origin = new vec3(0.0, 0.0, 0.0);
  }
  
  ray_get_ray(float_u, float_v) { // float
    return new ray(this.vec3_origin, this.vec3_lower_left_corner
      .add(this.vec3_horizontal.mulfl(float_u))
      .add(this.vec3_vertical.mulfl(float_v))
...
class raytracer {
  ...
  constructor(c) {
    ...
    // camera and scene
    this.cam = c.camera || new camera(90, this.nx/this.ny);
    this.world = c.world;
...
var ep014 = {
  ...
  renderSinglePass: function() {
    ...
    this.raytracer = new raytracer({
      ...
      camera: new camera(new vec3(-2,2,1), new vec3(0,0,-1), new vec3(0,1,0), 90, this.width/this.height),
