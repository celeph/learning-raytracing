...
class raytracer {
  ...
  color(r, world) {
    ...
    if (world.hit(r, 0.001, Infinity, rec)) {
      ...
      // var target = rec.p.add(rec.normal.add(this.random_in_unit_sphere()));
      // return this.color( new ray(rec.p, target.sub(rec.p)), world ).mulfl(0.5);
      
      var target = rec.normal.add(this.random_in_unit_sphere());
      return this.color( new ray(rec.p, target), world ).mulfl(0.5);
  ...
  renderRay(i,j) {
    ...
    if (this.antiAliasing) {
      for (var s = 0; s < this.ns; s++) {
        ...
        col = col.add(this.color(r, this.world));
      }
